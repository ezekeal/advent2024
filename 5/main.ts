...
const rules = r.split('\n').map(pair => pair.split('|').map(c => parseInt(c)));
const updates = o.split('\n').map(update => update.split(',').map(c => parseInt(c)));
const rulesRef = new Map();
const getRule = (n: number) => rulesRef.get(n) || { before: new Set(), after: new Set() };
const setRules = (n: number, {before, after}: {before?: number, after?: number}) => {
